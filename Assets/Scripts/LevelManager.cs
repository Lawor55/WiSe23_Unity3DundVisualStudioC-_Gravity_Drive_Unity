...
public class LevelManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (activeLevelModules[activeLevelModules.Count - 1].transform.position.x + (activeLevelModules[activeLevelModules.Count - 1].transform.GetChild(0).transform.localScale.x / 2) < 42)
        {
            ...
            //reenable all fuel tanks of newly enabled level modules
            Fuel_Tank[] fuelTanks = activeLevelModules[activeLevelModules.Count - 1].GetComponentsInChildren<Fuel_Tank>(true);
            foreach (var fuel_Tank in fuelTanks)
            {
                //print("Reenable Fueltank");
